...
from repositories.opportunity_repository import (
    find_all_opportunities,
    create_opportunity as create_opportunity_repo,
...
async def get_opportunities_paginated(
    ...
    return await find_all_opportunities(
        page=page,
        page_size=limit,
        search=search,
        order_by=order_by,
...
async def create_opportunity(data: Dict[str, Any]) -> Any:
    ...
    if not deal_id:
        deal = await get_or_create_deal("Opportunities Pipeline")
        deal_id = deal.id

    status_id = data.get("current_status_id")
    if not status_id:
        status_name = data.get("status", "Qualifying")
        status = await find_status_by_name(status_name, "opportunity")
    ...
    return await create_opportunity_repo(create_data)


async def get_opportunity(opp_id: str) -> Any:
    ...
    opp = await find_opportunity_by_id(opp_id_int)
    if not opp:
...
async def update_opportunity(opp_id: str, data: Dict[str, Any]) -> Any:
    ...
    allowed_fields = [
        "title", "opportunity_name", "estimated_value", "probability",
        "notes", "source", "project_ids",
    ]
    update_data.update({k: data[k] for k in allowed_fields if k in data})

    if "expected_close_date" in data:
        update_data["expected_close_date"] = _parse_date(data["expected_close_date"])

    if "current_status_id" in data:
        update_data["current_status_id"] = data["current_status_id"]
    elif "status" in data:
        status = await find_status_by_name(data["status"], "opportunity")
        update_data["current_status_id"] = status.id if status else None

    updated = await update_opportunity_repo(opp_id_int, update_data)
    if not updated:
        raise HTTPException(status_code=404, detail="Opportunity not found")
...
async def delete_opportunity(opp_id: str) -> bool:
    ...
    deleted = await delete_opportunity_repo(opp_id_int)
    if not deleted:
        raise HTTPException(status_code=404, detail="Opportunity not found")
