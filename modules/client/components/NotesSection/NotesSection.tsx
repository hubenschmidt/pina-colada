...
interface NotesSectionProps {
  ...
  // For create mode - collect notes to save after entity creation
  pendingNotes?: string[];
  onPendingNotesChange?: (notes: string[]) => void;
}
...
const NotesSection = ({
  ...
  const renderPendingNoteCard = (content: string, index: number) => (
    <div
      key={`pending-${index}`}
      className="p-3 border border-zinc-200 dark:border-zinc-700 rounded"
    >
  ...
  return (
    <div className="space-y-2">
      ...
      {/* Empty state */}
      {!isLoading && notes.length === 0 && (!pendingNotes || pendingNotes.length === 0) && !showAddForm && (
