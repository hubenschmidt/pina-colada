import { createJob, fetchLeadStatuses } from "../api";
 ...
 */
const JOB_LEAD_PATTERN =
  /(?:^\d+\.\s*)?(.+?)\s*-\s*(.+?)\s*-\s*(https?:\/\/[^\s]+)/gm;

/**
 * Extract job leads from message text
 */
export const extractJobLeads = (
  messageText: string
): Array<{
  company: string;
  job_title: string;
 ...
 * @returns Number of leads saved
 */
export const extractAndSaveJobLeads = async (
  messageText: string
): Promise<number> => {
